...
from hypersp.utils.helpers import dynamic_import
...
class AudioDataset(Dataset):
    def __init__(self,
        ...
        Manifest = dynamic_import(task_path+":Manifest")
    ...
    def __getitem__(self, index):
        ...
        return (
            features,
            torch.tensor(features.shape[0]).int(),
            torch.tensor(sample["transcript"]),
            torch.tensor(len(sample["transcript"])).int()
        )
