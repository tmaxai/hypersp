...
def normalize_batch(x, seq_len, normalize_type: str):
    ...
    elif isinstance(normalize_type, Dict):
        x_mean = torch.tensor(
            normalize_type["fixed_mean"], dtype=x.dtype, device=x.device)
        x_std = torch.tensor(
            normalize_type["fixed_std"], dtype=x.dtype, device=x.device)
...
class SpectrogramFeatures(nn.Module):
    ...
    __constants__ = ["dither", "preemph", "n_fft", "hop_length", "win_length", "log",
                     "frame_splicing", "window", "normalize", "pad_to", "max_duration", "do_normalize"]
...
class FilterbankFeatures(nn.Module):
    # For JIT. See https://pytorch.org/docs/stable/jit.html#python-defined-constants
    __constants__ = ["dither", "preemph", "n_fft", "hop_length", "win_length", "center",
                     "log", "frame_splicing", "window", "normalize", "pad_to", "max_duration", "max_length"]
