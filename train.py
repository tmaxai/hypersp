...
from hypersp.utils.helpers import (add_ctc_labels, model_multi_gpu, monitor_asr_train_progress,
                                   print_dict, print_once, process_evaluation_batch,
                                   process_evaluation_epoch)
from hypersp.models import Jasper
from hypersp.losses import CTCLossNM
from hypersp.modules import GreedyCTCDecoder
from hypersp.optimizers import Novograd, AdamW
...
def save(model,
    ...
    if rank == 0:
        ...
        if ema_model is not None:
            checkpoint['ema_state_dict'] = getattr(
                ema_model, 'module', ema_model).state_dict()
...
def train(data_layer,
    ...
    def eval(model, name=''):
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                ...
                t_loss_e = ctc_loss(log_probs=t_log_probs_e, targets=t_transcript_e,
                                    input_length=t_encoded_len_e, target_length=t_transcript_len_e)
    ...
    encoder = model.module.encoder if hasattr(
        model, 'module') else model.encoder
    ...
    while True:
        ...
        for data in train_dataloader:
            ...
            t_loss_t = ctc_loss(log_probs=t_log_probs_t, targets=t_transcript_t,
                                input_length=t_encoded_len_t, target_length=t_transcript_len_t)
            ...
            if batch_counter % args.gradient_accumulation_steps == 0:
                ...
                if step % args.train_frequency == 0:
                    ...
                    print_once(
                        "Loss@Step: {0}  ::::::: {1}".format(step, str(round(average_loss, 2))))
        ...
        if epoch % args.save_frequency == 0 and epoch > 0:
            save(model, ema_model, optimizer, epoch,
                 args.output_dir, optim_level)
...
def main(args):
    ...
    data_layer = AudioToTextDataLayer(
        ...
        normalize_transcripts=model_definition["input"]["normalize_transcripts"])
    ...
    data_layer_eval = AudioToTextDataLayer(
        ...
        multi_gpu=multi_gpu,
        pad_to_max=args.pad_to_max,
        normalize_transcripts=model_definition["input_eval"]["normalize_transcripts"])
    ...
    def fn_lr_policy(s): return lr_policy(
...
def parse_args():
    ...
    parser.add_argument("--eval_freq", dest="eval_frequency", default=200, type=int,
    ...
    parser.add_argument("--ema", type=float, default=0.0,
                        help='discount factor for exponential averaging of model weights during training')
